...
class _EstimationFormScreenState extends State<EstimationFormScreen> {
  ...
  void initState() {
    ...
    _clientVatController = TextEditingController(text: est?.clientVatNumber ?? '');
  ...
  Future<void> _saveEstimation() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (widget.estimation != null) {
        success = await provider.updateEstimation(estimation);
      } else {
        success = await provider.addEstimation(estimation);
      }
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(provider.errorMessage ?? 'Failed to save estimation'),
              backgroundColor: Colors.red.shade400,
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(10),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: _isSaving ? null : _saveEstimation,
                    child: Container(
                      width: 44,
                      height: 44,
                      decoration: BoxDecoration(
                  ...
                  child: Column(
                    ...
                    children: [
                                  ...
                                  child: GestureDetector(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          '${item.quantity} ${item.unit} x SAR ${item.unitPrice.toStringAsFixed(2)}',
                                          style: TextStyle(
                                            fontSize: 12,
                      ...
                      _buildTextField(
                        ...
                        label: 'Subject (e.g., Website Development Quote)',
...
class _ItemFormSheet extends StatefulWidget {
  ...
  const _ItemFormSheet({this.item, required this.onSave, required this.isDark});
...
class _ItemFormSheetState extends State<_ItemFormSheet> {
  ...
  // Available units
  final List<String> _units = ['unit', 'pcs', 'kg', 'g', 'liter', 'ml', 'hour', 'day', 'month', 'year', 'box', 'set', 'sqm', 'job'];
